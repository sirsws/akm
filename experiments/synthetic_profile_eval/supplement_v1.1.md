...
核心功能：v1.1 多生成器补充实验，回应 v1.0「单一生成器」局限。验证 AKM 优势在 6 个跨家族 LLM 生成器上是否普遍成立。
...
> **Scope:** This supplement does **not** introduce a new method. It repeats the v1.0 synthetic-profile evaluation across **six** generator LLMs from six different families, to test whether the AKM advantage observed under a single generator (DeepSeek-V4-pro) generalizes.
> **TL;DR:** AKM's profile-first conditions (`akm_profile`, `akm_elicited`) deliver **+13.8 to +19.8 points** over no-profile across **6/6** generators (raw 0–30 scale), including a generator (Gemma-4-31b-it) that ranks much lower on the OpenRouter leaderboard than the others, and a generator (GLM-5.1) whose absolute AKM ceiling sits visibly lower than the rest. The v1.0 finding is therefore **not an artifact of the original generator choice**.
...
To address both, we expanded from 1 generator to **6 cross-family generators** while keeping every other variable fixed (same 50 personas, same 4 conditions, same 3 cross-family judges, same rubric). No human calibration, no rubric redesign, no N>200. This stays inside the v1.0 design envelope.

---

## 2. Setup

### 2.1 Generators (6 families)
...
| `kimi_k2_6` | OpenRouter `moonshotai/kimi-k2.6` | Moonshot | 28 | new (partial — see §6) |
| `gemma_4_31b_it` | OpenRouter `google/gemma-4-31b-it` | Google (open weights) | 38 | new (partial — included as **deliberately weaker** generator to probe ceiling effects) |
...
## 3. Main result — AKM lift across 6 generators

Total score is the sum of 6 rubric dimensions, each on 0–5, so the per-judge total ranges 0–30. Pooled values below average over the 3 judges. `Δ AKM` is `akm_profile − no_profile`. `Δ Elicited` is `akm_elicited − no_profile`.
...
| kimi_k2_6 (partial) <!-- verify: generator=kimi_k2_6 stat=n_personas value=13 --> | 13 | 13.03 <!-- verify: generator=kimi_k2_6 condition=no_profile stat=total_mean value=13.026 --> | 23.40 <!-- verify: generator=kimi_k2_6 condition=unstructured_notes stat=total_mean value=23.395 --> | 26.84 <!-- verify: generator=kimi_k2_6 condition=akm_profile stat=total_mean value=26.842 --> | 28.42 <!-- verify: generator=kimi_k2_6 condition=akm_elicited stat=total_mean value=28.421 --> | **+13.82** <!-- verify: generator=kimi_k2_6 stat=lift_akm value=13.816 --> | +15.40 <!-- verify: generator=kimi_k2_6 stat=lift_elicited value=15.395 --> | ✓ |
| gemma_4_31b_it (partial) <!-- verify: generator=gemma_4_31b_it stat=n_personas value=13 --> | 13 | 11.11 <!-- verify: generator=gemma_4_31b_it condition=no_profile stat=total_mean value=11.105 --> | 19.50 <!-- verify: generator=gemma_4_31b_it condition=unstructured_notes stat=total_mean value=19.5 --> | 29.82 <!-- verify: generator=gemma_4_31b_it condition=akm_profile stat=total_mean value=29.816 --> | 29.68 <!-- verify: generator=gemma_4_31b_it condition=akm_elicited stat=total_mean value=29.684 --> | **+18.71** <!-- verify: generator=gemma_4_31b_it stat=lift_akm value=18.711 --> | +18.58 <!-- verify: generator=gemma_4_31b_it stat=lift_elicited value=18.579 --> | ✓ |

**Reading the table.**

- The **lift direction is uniform**: all 6 generators show `akm_profile ≫ unstructured_notes ≫ no_profile`. There is no generator for which AKM hurts or is even neutral. The smallest lift is +13.82 (Kimi, partial); the largest is +19.81 (Qwen).
- **Strong-model floor effect is rejected.** Gemma-4-31b-it is the lowest-ranked generator we tested (rank 38 vs rank 18–34 for the others) and yet, on its 13 partial personas, it shows a +18.71 AKM lift — the second largest of the six. AKM is not "free help only for top models".
- **Generator-AKM coupling is rejected.** AKM helps every family — DeepSeek (rank 24), OpenAI (33), Alibaba (25), Zhipu (18), Moonshot (28), Google open-weights (38). The protocol, not a particular family's idiosyncratic strength, drives the lift.
...
| Generator | α (total) | α range over 6 dims | Spearman pairs | n units |
|---|---:|---|---|---:|
| deepseek_v4_pro | **0.948** <!-- verify: generator=deepseek_v4_pro stat=alpha_total value=0.948 --> | 0.824–0.942 | 1.00 / 1.00 / 1.00 | 200 |
| gpt_5_4_mini | **0.894** <!-- verify: generator=gpt_5_4_mini stat=alpha_total value=0.894 --> | 0.697–0.933 | 0.95 / 0.95 / 1.00 | 200 |
| qwen_3_6_plus | **0.949** <!-- verify: generator=qwen_3_6_plus stat=alpha_total value=0.949 --> | 0.829–0.954 | 1.00 / 1.00 / 1.00 | 200 |
| glm_5_1 | **0.931** <!-- verify: generator=glm_5_1 stat=alpha_total value=0.931 --> | 0.742–0.939 | 1.00 / 1.00 / 1.00 | 176 |
| kimi_k2_6 (partial) | **0.859** <!-- verify: generator=kimi_k2_6 stat=alpha_total value=0.859 --> | 0.660–0.887 | 1.00 / 1.00 / 1.00 | 52 |
| gemma_4_31b_it (partial) | (computed at run time) | — | — | small |

All 5 fully-evaluated generators clear α > 0.85 — well above the 0.80 "strong agreement" threshold. The single per-dimension dip below 0.70 is `actionability` for both GPT-5.4-mini (0.697) and Kimi-K2.6 (0.660) — judges disagree slightly more on what "actionable enough" looks like in plain text, but never enough to flip rankings: every condition-mean Spearman ρ between any two judges is ≥ 0.95. **The v1.0 finding that judge family does not change the ranking is reproduced here for every new generator.**
...
### 6.1 Partial generators

- **Kimi-K2.6 (78/200 = 39% complete).** The OpenRouter Kimi endpoint repeatedly returned empty content for our long Chinese prompts even after retries. We stopped after 78 generations. From the 78 generations, only **13 personas** had all 4 conditions complete and could enter the analysis. We report Kimi as a partial generator and use n=13.
- **Gemma-4-31b-it (~80/200 at the time of this freeze).** OpenRouter's Gemma endpoint hit upstream rate limits (`HTTP 429` from Venice / Parasail / DeepInfra) intermittently. Generation continued in the background but at a heavily throttled pace. We report Gemma at the most recent aggregation snapshot (n=13 complete personas at ~80 generations); the script can resume and bring this number up if reviewers prefer.
- **GLM-5.1 (49/50 personas).** One persona (`fashion_007`, condition `akm_profile`) failed all 3 retries with empty content from OpenRouter. We dropped that persona for GLM only. Other generators are unaffected.

### 6.2 `format_repair` calls
...
Total invocations: **38** events out of ~600 attempted JSON-elicitation calls (≈6.3%). Almost all are for Kimi-K2.6 (truncated trailing strings) and GLM-5.1 (unterminated strings near the end of long traces). DeepSeek and GPT had 0 repairs; Qwen had 0 repairs; Gemma's repairs are bundled inside its own retries.
...
> The AKM advantage over no-profile and over unstructured notes — first reported in v1.0 with DeepSeek-V4-pro as sole generator — is reproduced across 6 cross-family generators (DeepSeek, OpenAI, Alibaba, Zhipu, Moonshot, Google open-weights), with `Δ AKM` ranging from +13.82 to +19.81 on the 0–30 rubric. The advantage holds for the strongest model in the set and for the weakest model in the set, and holds even on a generator (GLM-5.1) whose absolute AKM ceiling is visibly lower than the rest.
